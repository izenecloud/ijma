...
$JMA_PATH/bin/basis_test/go-seg.sh $RAW_SENTENCE $BASIS_OUTPUT >>$LOG 2>&1
RET=$?
if [ "$RET" != "0" ]
then
...
echo ">>>>> iJMA start" >> $LOG
$JMA_PATH/bin/jma_run --stream $RAW_SENTENCE $JMA_OUTPUT --dict $JMA_PATH/db/ipadic/bin_utf8 >>$LOG 2>&1
RET=$?
if [ "$RET" != "0" ]
then
